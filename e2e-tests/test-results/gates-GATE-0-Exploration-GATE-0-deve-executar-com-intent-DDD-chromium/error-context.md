...
- Name: gates.spec.ts >> GATE-0: Exploration >> GATE-0 deve executar com intent DDD
- Location: tests/gates.spec.ts:37:7

# Error details

```
Error: Command failed: rm -rf /tmp/devorq-e2e-sandbox && mkdir -p /tmp/devorq-e2e-sandbox
rm: cannot remove '/tmp/devorq-e2e-sandbox': Is a directory
